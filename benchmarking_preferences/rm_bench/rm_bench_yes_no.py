...
def extract_dataset_name(dataset_name):
    """Extract the last part of dataset name after the final slash"""
...
def save_entropy_data(processed_data, dataset_name, model_name):
    ...
    name = extract_dataset_name(dataset_name)
    model_short = model_name.split('/')[-1]
    
    filename = f"entropy_data_{name}_{model_short}.json"
...
def save_all_accuracies_to_json(all_accuracies, model_name):
    ...
    model_short = model_name.split('/')[-1]
    filename = f"accuracy-rm-bench-{model_short}-entropy.json"
    with open(filename, 'w') as f:
...
def save_combined_entropy_data(all_processed_data, model_name):
    ...
    model_short = model_name.split('/')[-1]
    filename = f"combined_entropy_data_{model_short}.json"
...
def main(args):
    ...
    for dataset_name in datasets:
        ...
        local_path = f"{name}-{model_short}-response-entropy"
        ...
        # processed_dataset.push_to_hub(f"{args.hf_user}/{name}-{model_short}-response-entropy")
